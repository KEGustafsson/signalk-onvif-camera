...
OnvifDevice.prototype._getSystemDateAndTime = function () {
  const promise = new Promise((resolve, _reject) => {
    this.services.device.getSystemDateAndTime((error, _result) => {
...
OnvifDevice.prototype._mediaGetProfiles = function () {
  const promise = new Promise((resolve, reject) => {
    this.services.media.getProfiles((error, result) => {
      ...
      // Clear profile list and current profile to prevent duplicates and stale references
      this.profile_list = [];
      this.current_profile = null;
      profiles.forEach((p) => {
        const profile = {
        ...
        if (p['PTZConfiguration']) {
          try {
            ...
            // PTZ X range not available for this camera
          }
          try {
            const r = p['PTZConfiguration']['PanTiltLimits']['Range'];
            const yr = r['YRange'];
            ...
            // PTZ Y range not available for this camera
          }
          try {
            ...
            // PTZ Zoom range not available for this camera
          }
        }
        ...
        this.services.media.getSnapshotUri(params, (error, result) => {
          ...
          if (!error) {
            try {
              ...
              console.error('Error parsing snapshot URI:', e.message);
            }
          }

          // For Hikvision cameras, construct channel-based snapshot URL from RTSP stream
          // This is needed because Hikvision cameras ignore the ONVIF profile parameter in snapshot URLs
          const manufacturer = (this.information && this.information.Manufacturer) || '';
          const isHikvision = manufacturer.toLowerCase().includes('hikvision');

          if (isHikvision) {
            const rtspUrl = profile['stream']['rtsp'];
            if (rtspUrl) {
              const channelMatch = rtspUrl.match(/\/Streaming\/Channels\/(\d+)/i);
              if (channelMatch) {
                const channelNum = channelMatch[1];
                // Construct Hikvision ISAPI snapshot URL based on channel number
                const isapiSnapshotUrl = `http://${this.address}/ISAPI/Streaming/channels/${channelNum}/picture`;
                profile['snapshot'] = isapiSnapshotUrl;
              }
            }
          }
