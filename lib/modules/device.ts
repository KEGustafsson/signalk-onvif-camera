...
OnvifDevice.prototype.fetchSnapshot = function (callback) {
  const promise = new Promise((resolve, reject) => {
    ...
    const ourl = new URL(this.current_profile['snapshot']);
    const options = {
      protocol: ourl.protocol,
      auth: this.user + ':' + this.pass,
      hostname: ourl.hostname,
      port: parseInt(ourl.port) || 80,
...
OnvifDevice.prototype._getXaddr = function (directXaddr) {
  ...
  try {
    const u = new URL(directXaddr);
    return 'http://' + this.address + u.pathname + u.search;
  } catch (_e) {
    return directXaddr;
  }
};
...
OnvifDevice.prototype._getUri = function (directUri) {
  ...
  try {
    ...
    return '//' + base.host + parts.pathname;
  } catch (_e) {
    return directUri;
  }
};
...
OnvifDevice.prototype._getSnapshotUri = function (directUri) {
  ...
  try {
    const base = new URL('http://' + this.address);
    const parts = new URL(directUri);
    return parts.protocol + '//' + base.host + parts.pathname;
  } catch (_e) {
    return directUri;
  }
};
