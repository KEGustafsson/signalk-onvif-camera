...
function Onvif() {
  ...
  this._probeInProgress = false; // Guard against concurrent probes
}
...
Onvif.prototype.startProbe = function (callback) {
  ...
  const promise = new Promise((resolve, reject) => {
    // Prevent concurrent probe operations
    ...
    this._udp.on('message', (buf, _device_info) => {
    ...
    this._udp.bind(() => {
      if(ipAddress !== undefined) {
        try {
          this._udp.setMulticastInterface(ipAddress);
        } catch (error) {
          console.error('Error, check ONVIF LAN IP address. Ignoring IP setting.');
        }
      }
...
Onvif.prototype._sendProbe = function (_callback) {
  let soap_tmpl = '';
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const send = () => {
      ...
      if(soap) {
        ...
        try {
          this._udp.send(buf, 0, buf.length, this._PORT, this._MULTICAST_ADDRESS, (_error, _bytes) => {
